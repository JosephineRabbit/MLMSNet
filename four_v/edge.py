...
class FeatLayer_ed(nn.Module):
    def __init__(self, in_channel, channel, k):
        ...
        self.ed = nn.Sequential(nn.Conv2d(channel+1,NN,1,1),nn.ReLU(inplace=True))
        #self.conv2 = nn.Sequential(nn.Conv2d(channel,channel))
        self.main2 =nn.Sequential(nn.Conv2d(channel, channel-NN, k, 1, k // 2), nn.ReLU(inplace=True),
                                  nn.Dropout())
        self.o =nn.Conv2d(channel, 1, 1, 1)
        self.o2 = nn.Conv2d(channel, 1, 1, 1)
...
class Edge_featlayer_2(nn.Module):
    def __init__(self,in_channel,channel):
        ...
        self.conv2 = nn.Sequential(nn.Conv2d(in_channel,channel,1,1,dilation=1),nn.ReLU(inplace=True),nn.Conv2d(channel,channel,1,1,dilation=1),nn.ReLU(inplace=True))
...
class Edge_featlayer_3(nn.Module):
    def __init__(self,in_channel,channel):
        ...
        self.conv3 = nn.Sequential(nn.Conv2d(in_channel,channel,1,1,dilation=1),nn.ReLU(inplace=True),nn.Conv2d(channel,channel,1,1),nn.ReLU(inplace=True))
...
class D_U(nn.ModuleList):
    ...
    def forward(self, features):
        ...
        mask.append(nn.Sigmoid()(self.extract0(torch.cat([x0,features[4]],1))))
        x1 = self.up0(torch.cat([x0,features[4]],1))
        e.append(nn.Sigmoid()(self.extract1(x1)))

        x2 = self.up1(torch.cat([features[3],x1],1))
        mask.append(nn.Sigmoid()(self.extract2(x2)))
        x3 = self.up2(torch.cat([features[2],x2],1))
        e.append(nn.Sigmoid()(self.extract3(x3)))
        ...
        mask.append(nn.Sigmoid()(self.extract_f_m(torch.cat([features[0],x4],1))))
...
class Edge_featlayer_2(nn.Module):
    def __init__(self,in_channel,channel):
        super(Edge_featlayer_2,self).__init__()
        self.conv1 = nn.Sequential(nn.Conv2d(in_channel,channel,1,1,dilation=1),nn.ReLU(inplace=True))
        self.conv2 = nn.Sequential(nn.Conv2d(in_channel,channel,1,1,dilation=1),nn.ReLU(inplace=True))
        self.merge = nn.Conv2d(2*channel,1,1)
...
class Edge_featlayer_3(nn.Module):
    def __init__(self,in_channel,channel):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(in_channel, channel, 1, 1, dilation=1), nn.ReLU(inplace=True))
        self.conv2 = nn.Sequential(nn.Conv2d(in_channel, channel, 1, 1, dilation=1), nn.ReLU(inplace=True))
        self.conv3 = nn.Sequential(nn.Conv2d(in_channel,channel,1,1,dilation=1),nn.ReLU(inplace=True))
        self.merge = nn.Conv2d(3 * channel, 1, 1)
