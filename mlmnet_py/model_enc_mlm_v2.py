...
class DSS(nn.Module):
    def __init__(self, base, feat_layers,e_feat_layers):
       ...
       # self.feat_2 = nn.ModuleList(feat_layers)
        ...
        self.up_sal_0  = nn.ModuleList()
        self.up_sal_1 = nn.ModuleList()
        self.up_sal_2 = nn.ModuleList()
        self.up_sal_e =nn.ModuleList()

        self.up_e.append(nn.Conv2d(1,1,1))
        self.up_sal_0.append(nn.Conv2d(1, 1, 1))
        self.up_sal_1.append(nn.Conv2d(1, 1, 1))
        self.up_sal_2.append(nn.Conv2d(1, 1, 1))
        ...
        for i  in range(5):
            ...
            if i<4:
                ...
                #k = 2 * k
            self.up_sal_0.append(nn.ConvTranspose2d(1, 1, k2, k2))
            self.up_sal_1.append(nn.ConvTranspose2d(1, 1, k2, k2))
            self.up_sal_2.append(nn.ConvTranspose2d(1, 1, k2, k2))
...
#            print(y2[i].shape,'========--------')
            ...
            m.append(self.up_sal_0[i](y2[i]))
            m_1.append(self.up_sal_1[i](m1_y2[i]))
            m_2.append(self.up_sal_2[i](m2_y2[i]))
            #print(m[i].shape)
            m[i] = nn.Sigmoid()(m[i])
...
if __name__ == '__main__':
   ...
   # net2 = D_U()#.cuda()
    ...
    weights_init_kaiming_n(net.net.up_sal_0)
    weights_init_kaiming_u(net.net.up_sal_1)
    weights_init_xav_u(net.net.up_sal_2)
   ...
   #net.net.base.load_state_dict(torch.load('vgg_weights/vgg16.pth'))
    ...
    m,e, m_,m_1,m_2,e,edges = net(x,xe)
    print(len(m))
    #print(len(e))
    #
    # for i in f_1:
    ...
    for i in m:
        print('dec mask',i.shape,'sal_pred')
    #
    for e_ in e:
        print('dec e',e_.shape)


    for mm in m_:
        print('enc m0',mm.shape)

    for mm in m_1:
        print('enc m1',mm.shape)

    for mm in m_2:
        print('enc m2',mm.shape)
    #
    for i in edges:
        print('enc edge', i.shape, 'edge_pred')
