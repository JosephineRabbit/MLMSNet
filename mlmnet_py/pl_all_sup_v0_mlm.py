...
class PytorchLightningModel(pl.LightningModule):  # 這邊一定要繼承pl.LightningModule
    ...
    def mlm_loss(self,m,m_1):
        loss = 0
        
        for i in range(len(m)):
            loss+=nn.MSELoss()(m[i],m_1[i].detach())
    ...
    def training_step(self, batch, batch_idx):  # 定義訓練過程的Step要如何進行
      ...
      #  x, y = batch  # 從self.train_dataloader()的Data Loader取一個batch出來。
        ...
        if self.iter_cnt ==0 :
            weights_init_kaiming_u(self.net.net.feat)

            weights_init_kaiming_n(self.net.net.feat_1)
            weights_init_xav_u(self.net.net.feat_2)
            mlm_loss_v = 0
        ...
        if self.iter_cnt!=0:
            if self.iter_cnt%3==0:
                mlm_loss_v = (self.mlm_loss(m,m_1)+self.mlm_loss(m,m_2))/12
            if self.iter_cnt%3==1:
                mlm_loss_v = (self.mlm_loss(m_1,m))/6
            if self.iter_cnt%3==2:
                mlm_loss_v = (self.mlm_loss(m_2,m))/6
        output = self.forward(img,ed_img)
        (m, m_1, m_2, e, edges, m_dec, e_dec) = output
        criterion =  torch.nn.BCELoss()#nn.functional.binary_cross_entropy
    #    print(m_dec[2].max(),m_dec[2].min())
      ...
      #      print(m_e_d.shape)
        ...
        loss = mlm_loss_v+en_loss +dec_loss+5*criterion(m_dec[2], target)
    ...
    def validation_epoch_end(self,outputs):  # 在Validation的一個Epoch結束後，計算平均的Loss及Acc.
       ...
       # tensorboard_logs = { 'avg_val_mae': avg_val_mae,'best_mae':self.best_mae}
        fd = open("all_sup_v0_mlm_1.txt",'a+')
