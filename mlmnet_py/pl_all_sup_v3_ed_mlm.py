...
from model_ent_v0404 import DSE, D_U, initialize_weights, weights_init_kaiming_u, weights_init_kaiming_n, weights_init_xav_u
...
class PytorchLightningModel(pl.LightningModule):  # 這邊一定要繼承pl.LightningModule
    ...
    def forward(self, x,e):  # 定義模型在forward propagation時如何進行.
        output= self.net(x,e)
        return output
    def cal_mlm_loss(self,m,m_1):
        mlm_loss = 0
        for i in range(6):
            mlm_loss += nn.MSELoss()(m[i],m_1[i].detach())/6
        return mlm_loss
    def training_step(self, batch, batch_idx):  # 定義訓練過程的Step要如何進行
        ...
        if self.flag_begin:
            ...
            weights_init_kaiming_u(self.net.net.feat_1)
            weights_init_kaiming_n(self.net.net.feat_2)
            weights_init_xav_u(self.net.net.feat)
    ...
    #    print(m_dec[2].max(),m_dec[2].min())
      #  for m_e_d in m_dec:
      #      print(m_e_d.shape)i
        if self.iter_cnt%3==0:
            if self.flag_begin:
                mlm_loss=0
            else:
                mlm_loss = (self.cal_mlm_loss(m,m_1)+self.cal_mlm_loss(m,m_2))/2
        if self.iter_cnt%3==1:
            mlm_loss = (self.cal_mlm_loss(m_1,m_2)+self.cal_mlm_loss(m_1,m))/2
        if self.iter_cnt%3==2:
            mlm_loss = (self.cal_mlm_loss(m_2,m_1)+self.cal_mlm_loss(m_2,m))/2
        edge_loss = nn.BCELoss()(edges[-1],ed_target)
        ...
        en_loss +=(criterion(m_1[2], e_target)+criterion(m_1[0], e_target)+criterion(m_1[4], e_target)+criterion(m_1[1], target)+criterion(m_1[3], target)+criterion(m_1[5], target))/6
        en_loss +=(criterion(m_2[2], e_target)+criterion(m_2[0], e_target)+criterion(m_2[4], e_target)+criterion(m_2[1], target)+criterion(m_2[3], target)+criterion(m_2[5], target))/6
        if self.iter_cnt<40000:
            dec_loss = criterion(e_dec[0], e_target)
        elif self.iter_cnt<80000:
            dec_loss = (criterion(e_dec[0], e_target)+criterion(e_dec[1], e_target))
        elif self.iter_cnt<120000:
            dec_loss = criterion(e_dec[0], e_target)+criterion(e_dec[1], e_target) +criterion(m_dec[0], target)
        elif self.iter_cnt<160000:
            dec_loss = criterion(e_dec[0], e_target)+criterion(e_dec[1], e_target) +criterion(m_dec[0], target)+criterion(m_dec[1], target)
        else:                
            dec_loss = (criterion(e_dec[0], e_target) +criterion(e_dec[1], e_target) +criterion(m_dec[0], target)+criterion(m_dec[1], target) +criterion(m_dec[2], target))/3+5*criterion(m_dec[2], target)
        loss = en_loss +dec_loss#+mlm_loss
    ...
    def validation_epoch_end(self,outputs):  # 在Validation的一個Epoch結束後，計算平均的Loss及Acc.
        ...
        if avg_val_mae<self.best_mae:
            ...
            torch.save(self.net.state_dict(),'new_edc3_all_sup_v3_3b_6_3_5_ed_0_mlm_0.pth')
        print(self.best_mae,self.current_epoch)
       # tensorboard_logs = { 'avg_val_mae': avg_val_mae,'best_mae':self.best_mae}
        fd = open("new_edc3_all_sup_v3_6_3_5_ed_0_mlm_0.txt",'a+')

        fd.write('\n %d epoch val mae %f best mae %f'%(self.current_epoch,avg_val_mae, self.best_mae))
